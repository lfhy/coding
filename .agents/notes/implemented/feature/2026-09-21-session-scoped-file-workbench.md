...
`dsh-client-ui-open-in-app` 把页头 entry 与工作台 entry 绑定到同一个 Session scope store：文件标签、当前文件、文件侧栏显隐、筛选、展开目录键和已加载目录都由该 store 持有。页头 entry 只负责打开工作台；最大化、终端底栏、文件侧栏和关闭作为常驻视图控制留在工作台顶栏右侧。最大化（以及窄屏自动进入的全屏呈现）会把整个会话列移出可见树，控制若挂在页头就会连同退出最大化、切换底栏与侧栏的入口一起消失，因此它必须随工作台本身渲染。文件树根目录在工作台首次显示后才请求，隐藏 entry 不再留下会话未就绪时的读取错误。终端底栏的 xterm viewport 使用文本光标，显示后立即聚焦。
...
代价是布局 store 增加按 Session 清理职责，页头 entry 与工作台 entry 共用一个 store handle，且文件树目录缓存随 Session 存活。覆盖验证见 `ui-layout` 的 AppFrame／store／service 测试和 `ui-open-in-app` 的页头、工作台与终端测试——其中工作台顶栏用例点击最大化、底栏、侧栏和关闭，钉住常驻控制与注入的布局动作接线；终端加载期光标由 CSS 直接约束。
