...
每次同步在请求下一页前拒绝重复的非空续传游标，并保留上一世代工具。空页无法通过工具名称唯一性证明分页在前进，因此游标记录还会检测跨越多页的循环。游标记录只属于一次同步，后续更新可以复用相同游标。桥接测试覆盖循环拒绝、保留可调用工具、严格启动失败和通知恢复。此机制检测重复游标，但不限制持续返回不同游标的服务器。
