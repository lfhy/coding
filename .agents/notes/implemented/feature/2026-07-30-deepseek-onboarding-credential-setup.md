...
**首次使用弹窗行内渲染既有提供方编辑器。** 适配器已挂载且处于活跃状态，其引用可解析、可写但尚未配置时，`ProviderEditor` 会渲染在共用引导弹窗中，并默认展开自定义设置，使 API 地址和模型目录直接可见。同一个组件全权负责 API 密钥校验、提供方设置的路径式 `settings.mutate` 写入、`credentials.set({ref, value})`、写入失败处理和写入后刷新。「稍后配置」只完成协调器当前这一轮。适配器缺失时仍跳过，因为浏览器不能挂载缺失的 Cordis 插件。
...
**在首次使用引导中单独实现表单**：不予采用，因为这会复制 Models 编辑器的 API 密钥草稿、模型校验、settings 写入、错误和已配置状态收敛。弹窗直接渲染既有 `ProviderEditor`。
...
有序流程从产品声明页开始，无需重启即可进入行内提供方表单：无密钥浏览器测试在隔离的 harness 家目录下启动真实 Web 组合，确认声明后从共用弹窗把生成的密钥存入该目录的 `.credentials.yaml`，并将 API 地址和模型容量写入 `settings.yaml`；测试还验证密钥未进入 DOM、ARIA 或浏览器控制台输出，并确认普通 Models 页面报告已配置。完整的无密钥 Web 回放也固定了同 id 的不可配置回放路由不会阻塞无关流程。纯就绪状态测试与 React 测试固化了受管文件凭据与进程环境凭据、提供方与能力缺失、取消、外部失效、协调器移交，以及 API 地址、模型和凭据的协同写入。该流程直接继承配置平面已记录的基础限制，不会另加局部的机密存储、脱敏或设置替换变通方案。
