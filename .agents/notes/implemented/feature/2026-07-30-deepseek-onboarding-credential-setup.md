...
**The prompt renders the existing provider editor inline.** A mounted, active adapter with a resolved, writable, unconfigured reference renders `ProviderEditor` inside the shared onboarding modal with custom settings open by default, so its Base URL and model catalog are immediately visible. The same component owns API-key validation, path-addressed `settings.mutate` writes for provider settings, `credentials.set({ref, value})`, write failures, and post-write refresh. Configure later completes only this coordinator pass. An absent adapter remains skipped because the browser cannot mount a missing Cordis plugin.
...
**A standalone onboarding implementation** — rejected because it would duplicate the Models editor's API-key draft, model validation, settings writes, errors, and configured-state convergence. The modal renders the existing `ProviderEditor` instead.
...
The ordered flow leads from the product notice to an inline provider form without restarting: a keyless browser test boots the real Web composition under an isolated harness home, acknowledges the notice, stores a generated key from the shared modal into the home's `.credentials.yaml`, persists an endpoint and model capacity into `settings.yaml`, verifies no key reaches DOM, ARIA, or browser console output, and confirms the ordinary Models page reports configured. The full keyless Web replay lane also pins that a non-configurable replay route with the same provider id does not block unrelated journeys. Pure readiness and React tests pin managed-file and process-environment credentials, missing providers and capabilities, cancellation, external invalidation, coordinator transfer, and coordinated endpoint/model/credential writes. The flow deliberately inherits the configuration plane's documented base limitations rather than adding local secret storage, redaction, or settings replacement workarounds.
