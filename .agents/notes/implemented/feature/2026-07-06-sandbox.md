...
- **Does the sandbox restrict network or process visibility?** The bwrap profile isolates its PID namespace, so a confined command sees only its own process tree; `SandboxMode` still claims FILE effects only, and no backend claims network. Whether network restriction becomes its own knob is left open in § The seam.
