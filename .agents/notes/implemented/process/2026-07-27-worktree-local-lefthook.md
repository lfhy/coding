...
Every dependency installation runs the root [`postinstall`](../../../../package.json). When the Lefthook development dependency is present, [`install-lefthook.mjs`](../../../../scripts/install-lefthook.mjs) invokes `lefthook install --force`. Linked Git worktrees otherwise share the common repository's default hooks directory, so an install in any worktree can rewrite hooks used by every other worktree.
...
Hook installation is worktree-scoped. With `CI=true` or `GITHUB_ACTIONS=true`, the installer returns before dependency or Git discovery because automated jobs do not consume contributor hooks. A production-only install likewise returns before Git discovery or mutation when pnpm omits the `lefthook` development dependency. Otherwise, the installer requires Git 2.26 or newer so `git config --show-scope` can report which scope supplied a value, upgrades a format-0 repository to format 1, enables `extensions.worktreeConfig`, and assigns the current worktree an absolute `core.hooksPath` at `$GIT_DIR/dsh-hooks`.
...
**Move Lefthook into production dependencies.** Runtime closures and production-only workspace installs do not consume contributor hooks. Shipping the hook manager would enlarge those installations to satisfy a development-only lifecycle script; treating absence as a no-op preserves the dependency boundary.
...
`pnpm install --production` neither configures hooks nor fails solely because Lefthook is omitted. Returning to a full development install makes the next `postinstall` configure the current worktree again.
