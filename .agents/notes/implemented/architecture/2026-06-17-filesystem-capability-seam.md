...
The first backend defaults to the host filesystem. For a current desktop Remote-SSH marker target, `dsh-fs-local` routes semantic filesystem operations to the selected remote root through the Go execution-world agent; it revalidates the four-part marker identity for every request and never falls back to the host path. [Desktop Remote-SSH uses a Go execution-world agent](../feature/2026-08-31-desktop-remote-ssh-go-execution-world.md) owns that route. Future sibling backends can provide sandboxed, remote, virtual, or project-scoped filesystems behind the same interface.
