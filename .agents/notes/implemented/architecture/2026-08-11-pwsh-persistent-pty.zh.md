...
**终端协议响应先于调用方输入。** headless emulator 不保留 scrollback，也不贡献模型可见文本；它维护终端控制状态，并经已挂载的 subprocess provider 写入响应。新增的 `@xterm/headless` 是受维护的运行时依赖，可避免光标查询吞掉后续工具命令。

**携带的风险。** Windows ACL 沙箱只读模式下，ConstrainedLanguage 可能拒绝引导代码通过 `[Console]::` 固定编码并写入 prompt marker；若 marker 就绪始终不可用，启动会在 `timeoutMs` 到期时拒绝，而不会发布引导未完成的 shell。模型后来重定义 `prompt` 函数会使命令就绪降级到静默档。模型命令中的裸 ESC 字符不受支持（PSReadLine 会吞掉）。koffi 和 `@xterm/headless` 分别增加进程基座与终端后端的依赖评审。
