...
macOS 桌面包将原始 Node 可执行文件放在 `Coding.app/Contents/Resources/coding-host`，将预展开 Host 闭包放在 `Coding.app/Contents/Resources/runtime`，并直接启动该闭包的 `bin.js`。Linux 终端启动器内嵌 Node SEA 引导资产。它会先将经过校验的生产闭包和所需原生侧车物化到 `$DSH_HOME/runtime/<sha256>`，再启动 Host。目录名是归档内容哈希，因此后续产品版本若携带相同字节会复用该目录。成功启动后删除旧运行时目录，启动失败则保留它们。两种形式都让最终用户无需预装 Node，同时为原生侧车保留真实路径。
...
- 发布产物不需要全局安装 Node 可执行文件：macOS 直接启动应用包闭包，Linux 则将 SEA 闭包物化到 `$DSH_HOME/runtime/<sha256>`。
- 第一方工作流仍由 Node Host 拥有；仅浏览器可用的第三方客户端视图在 TUI 中显示为不可执行占位。

## Risks

- Host 发现属于跨进程所有权协议。需要启动器锁和记录 token 清理，避免并发启动器为同一 `$DSH_HOME` 创建竞争写入者。
- 原生模块需要文件。macOS 包在应用闭包中保留它们，Linux SEA 路径物化平台侧车；两条路径都不声称存在纯通用单文件。
