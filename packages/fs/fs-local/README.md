...
`ctx.fs` 提供方约定（[`@deepseek-ai/dsh-fs`](../fs)）的**本地文件系统实现**。它使用宿主文件系统支持十二个 `FileSystem` 原语；将其作为插件加载会填充 `ctx.fs`。在桌面端组合中，它识别无凭据的 Remote-SSH Workspace marker，并经已认证的本地 bridge 将文件系统原语路由到所选远程根目录。`basic` 模式经 SFTP 读写，`agent` 模式经远端 agent 读写；普通本地路径仍保留宿主文件系统行为。模式与能力定义见 [`dsh-subprocess`](../../subprocess/subprocess/README.md)。
...
- **`resolve(path, opts?)`**：相对 `path` 在调用方提供 `opts.cwd` 时以该值为基准解析（面向模型的工具会传入调用 agent（智能体）的会话 cwd；见每会话 cwd 设计记录），否则以 `config.cwd` 为基准（默认 `process.cwd()`）；绝对 `path` 会忽略两者。`opts.signal` 会在解析前后检查。对于本地目标，`targetKey` 是文件的 `realpath`，因此经符号链接到达同一文件的两个输入路径会共享一个身份，写入／编辑落在链接目标上，同时保留链接；尚不存在的路径在父目录存在时使用 realpath 后的父目录加 basename，只有父目录无法解析时才回退到绝对路径。Remote-SSH marker 则生成 `coding-remote-target:v1:` 身份，包含连接模式，并把规范远程路径作为 `displayPath`；后续每次 I/O 都会针对 marker 当前连接和模式重新验证该身份。marker 缺失或失效的远端目标不会回退到本地路径。
...
- **`writeText`**：对于本地目标，原子发布会向目标旁随机命名的私有暂存目录（`0o700`）中排他打开的临时文件（`wx`、`0o600`）写入，随后执行 fsync 并发布。现有文件的 mode 会保留，新文件默认为 `0o600`；Windows 上的新文件继承目标目录的 DACL，而替换会在写入前把目标 DACL 复制到空临时文件，并通过 `ReplaceFileW` 发布，使原访问策略得以保留（见 Windows DACL 保留 设计记录）。`expected` 防护是可选的：省略时无条件创建或覆盖；`createIfAbsent` 通过硬链接把暂存文件发布到目标位置，以实现原子且不替换的发布，因此初始探测后创建的普通文件会被保留，并以 `FS_NOT_OBSERVED` 拒绝本次写入；非普通路径条目也会被保留，并以 `FS_NOT_REGULAR_FILE` 拒绝；`replaceIfVersion` 只在观察到的版本上替换（目标缺失或版本不匹配均为 `FS_STALE_VERSION`）。仅当打开后的旧文件和 UTF-8 替换内容都严格低于 `config.diffBasisMaxBytes`（默认 10 MiB）时，本地覆写才返回旧文本作为上下文 diff 基础；否则返回 `before: null`。Remote-SSH 把相同意图及规范化 before／after 结果委托给 bridge，并在发出请求后等待其确定确认；`basic` 使用 SFTP 执行，不会写入本地文件。
- **`editText`**：执行字面量读取、修改和写入，并通过变更锁按目标串行化；本地目标使用与 `writeText` 相同的发布原语，Remote-SSH 则把变更及规范化 before／after 结果委托给 bridge，并在发出请求后等待其确定确认。`basic` 使用 SFTP 执行，不会修改本地文件。`expected` 防护是可选的：提供时，会在字面量匹配之前校验版本（陈旧编辑报告 `FS_STALE_VERSION`，绝不会针对较新内容报告 `FS_EDIT_NOT_FOUND`/`FS_AMBIGUOUS_EDIT`）；省略时，无条件编辑当前内容。无论哪种情况，目标缺失都报告 `FS_STALE_VERSION`。两条路径都会在匹配时规范化为 LF，并拒绝空 `oldString`／零匹配（`FS_EDIT_NOT_FOUND`）或未设置 `replace_all` 的多个匹配（`FS_AMBIGUOUS_EDIT`）；本地存储还会恢复文件主要的 CRLF/LF 风格。
...
- **`config.cwd` 不是沙箱**：它是解析默认值，而非约束；绝对路径和 `..` 可以逃逸。请使用更严格的 `ctx.fs` 后端或 `tools/execute` waterfall（瀑布式事件）上的权限插件实施约束（见能力 seam 设计记录）。
...
- **基础模式的 SFTP 写入依赖服务端扩展**：`createIfAbsent` 需要 hardlink，覆写和编辑需要 posix-rename；不支持时 bridge 返回 501，本提供方以 `FS_IO_ERROR` 指明所缺扩展，不会改走本地文件系统。16 MiB 内普通文件的读取和带根约束的 stat／resolve 使用内容摘要版本，stat 因此会额外读取文件；目录列表和较大文件只给不可用于条件写入的元数据版本，写入前须先读取可支持的文件。`replaceIfVersion` 在提交前重新检查内容版本，但 SFTP 不提供原子的版本比较交换；并发修改可能发生在复核和发布之间。
