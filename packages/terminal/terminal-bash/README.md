...
基础 SSH 模式只在已验证的远端 target 上跳过前台检查：该模式的 SSH PTY 不提供前台进程组，因此可以发送输入、保留输出并在观察到输出后的静默期返回 `inferred_idle`，但不能以提示符推断出 `stdin_read` 或证明命令已完成。Agent 模式及本地会话仍要求发送前和就绪轮询的前台检查成功。基础模式仍受相同的沙箱策略限制：远端 shell 只允许 `danger-full-access`，不回退到本机 PTY。
...
- 基础 SSH 的静默就绪既不能证明命令边界，也不能确认前台信号的接收进程组；该连接的信号请求会以终端状态未知失败，取消不能伪装成可靠的 `SIGINT`。需要可靠的前台控制和中断时使用 Agent 模式。基础 SSH 目前只提供 POSIX bash PTY；pwsh 引导要求可证实的 `stdin_read`，不能以静默推断替代。
