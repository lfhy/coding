/** 通过 subprocess 的终端原语提供有界输出、就绪检测和终端协议响应的持久 PTY 会话。 */
...
// Node 只将该包的 CommonJS main 暴露为默认导出，因此经 require 读取具名导出。
...
export class LocalPtySession implements TerminalBackendSession {
  ...
  /** 仅维护终端协议状态；返回文本仍由 sanitizer 与有界缓冲区拥有。 */
  ...
  // TODO(pty-send-state-consolidation): 将下列每次发送字段
  // （active/activeTimer/activeDeadlineTimer/activeAbort/interrupting/
  // activeWrite/pollingReady/polling 与终端协议工作）收拢到单一生命周期所有者；
  // 取消与就绪的交互已有足够的固定测试支撑这项重构。
  ...
  /** 在下一次发送或就绪发布前，等待协议响应抵达 provider。 */
  ...
  /** 仅在整次前台检查期间协议响应保持完全停稳时，才接受该前台状态。 */
