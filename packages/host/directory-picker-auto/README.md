...
- **Not the shipped default** — the shipped web composition mounts `-browse` directly, because the native panel cannot take interaction when the host runs as a background child of the desktop shell. This chooser remains the opt-in for a deployment that wants the adaptive choice, and inherits the detection limits above.
