...
展开的品牌行渲染 `sidebar.brand.name`，收起轨道则渲染 `sidebar.brand.mark`。没有占位者时，外壳显示 `Coding` 标签和共享的 Coding 图标。部署包可以单独替换任一值，而无须替换 New Session 控件或轨道几何；声明感知的 `slots.inject()` 让这种包无论先于还是后于侧边栏激活都能生效。
