...
品牌行在宽侧栏渲染 `sidebar.brand.name`，并在名称与收起控件之间声明 `sidebar.brand.action` 这个通用 list slot：宽侧栏是横向图标行，56px rail 中排列在收起控件上方。注册者会收到 `wide`，且该 seat 在显示时必须适应宽栏图标行与 36px rail 控件盒。欢迎页操作行可见时该 seat 暂不渲染，侧边栏自身的收起按钮始终保留，折叠后仍可从 rail 展开。部署包可以替换展开状态的名称，而无须替换 New Session 控件或 rail 几何；声明感知的 `slots.inject()` 让这种包无论先于还是后于侧边栏激活都能生效。
...
侧边栏的收起与展开由品牌行自身的按钮调用 `ctx.layout.toggleSidebar()`；品牌行侧栏图形经 [`ui-icons`](../ui-icons/README.md) 的语义入口渲染。

桌面壳注入窗口拖拽样式时，展开侧栏的品牌行空白处与 macOS 交通灯右侧顶部空白可拖动窗口；品牌、操作按钮和折叠 rail 内的交通灯区域不接管原生拖拽。浏览器缺省无拖拽区域。
