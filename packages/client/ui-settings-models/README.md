...
声明完成后，DeepSeek 步骤会从同一个 Models 联接快照得出首次运行就绪状态。只要用户已经能触达**任何**一个提供方，它就直接完成而不渲染——已注册且其具名凭据引用已存储的路由（包括来自启动环境且只读的凭据），或 profile 根本不指名引用、因而走原生认证的路由。只有二者皆无的用户才会被要求填写 DeepSeek 官方密钥。适配器已挂载且活跃、引用可写但尚未配置时，既有 `ProviderEditor` 会在共用引导弹窗中完整渲染并默认展开自定义设置；API 地址和模型目录通过 `settings.mutate` 保存，`credentials.set` 仍是唯一的 secret 写入。「稍后配置」只完成协调器当前这一轮。适配器缺失、路由不活跃、联接失败、部署只读或设置／凭据能力不可用时，该步骤不渲染并直接完成；Models 页仍是诊断界面。
...
DeepSeek 官方提供方的渠道名称位于 API 密钥旁，在首次引导与 Models 页共用的编辑器中均可修改。未配置时显示 `default`；提交时去掉首尾空白，空白或超过 64 个字符的值会在字段旁提示而不写入。手动输入 `default` 可覆盖已保存的自定义名称；未改名称而只保存密钥时无需额外写入设置。该名称仅是单一渠道的显示元数据，不会创建新路由，也不改变 `deepseek-official` 或凭据引用 `DEEPSEEK_API_KEY`。
...
- **卡片只编辑 API 密钥、DeepSeek 渠道名称与精选折叠区字段**：手写编辑器用 schema 通用的字段覆盖面换来了设计稿上的布局（设计记录）。两个家族都公开 `baseURL` 与模型的 `id`/`name`/`contextWindow`/`maxTokens`；手工声明的 pi-ai 路由还公开 `displayName` 与 `api`。重试策略、超时、DeepSeek 模型说明及其他进阶字段仍留在 `settings.yaml` 中；编辑器未展示的现有模型字段会予以保留。不带这些约定字段的 profile schema 只渲染该提示，两套精选布局则以 `llm-deepseek`/`llm-pi-ai` 这两个 namespace 的名字为键。
