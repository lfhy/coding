...
Hero 选择器通过全局 `useWorkspaces` hook 列出真实的 Host Workspace 实体，并在输入时按标题或路径筛选。选择 Workspace 会调用 slot owner 的 `onPick` 回调，重新定位前端 Session 对象。不同的规范化路径即使 basename 和显示标题相同，仍会作为由 id 区分的独立 Workspace；侧边栏的悬停详情把 POSIX 家目录及其后代显示为 `~`／`~/…`，Windows 路径保持原样。其固定操作为**打开文件夹**、**远程连接**和**不在项目中工作**。第一项仅在目录流 slot 已被占用时渲染。远程连接的桌面向导先选择模式、填写 SSH 与密码或私钥认证，再观察连接进度并选择目录：默认基础模式只用 SSH + SFTP，允许远程文件读写编辑、搜索、前后台 Bash、PTY 和 Code Mode 的远端工具 binding，LSP 不可用，也无需上传远端 agent 和 SSH TCP 转发；Agent 模式具备上述能力及 LSP，需要上传仅监听远端回环地址的小型 Go agent（不是 Node），并获准 SSH direct-tcpip 转发。桌面端返回的 ready 模式是能力权威值；模式只在配置阶段可切换，切换会清理认证材料和原模式的待处理连接。Electron 通过 preload 注入的受限 API 授权；普通浏览器没有远程桥接调用面。认证信息只保留于当前对话框／连接；未知 `known_hosts` 密钥必须由用户明确确认，已变更的密钥会作为连接失败报告。选择目录会先创建不含凭据的本地 marker，再创建 Workspace，由本地 Host 将已获准能力路由到该目录。应用重启后不会恢复连接；重新执行同一远程连接流程会在不保存凭据的前提下把既有 marker 与 Workspace 重新绑定。无项目操作会在 Host 用户 HOME 目录创建并打开一个未分组 Session，因此「不在项目中工作」表示没有 Workspace 注册记录，而不是没有工作目录。侧边栏的仅添加入口仍只执行目录操作。
...
- **远程连接需要存活的桌面连接**：本地 Host 仍负责工具审批和持久 Session 日志。过期 marker 或断开的 bridge 会失败，不会把操作转向本机；桌面端重启后需要重新连接。
- **基础模式依赖 SFTP 扩展与 SSH 执行语义**：原子创建需要服务端 hardlink 扩展，覆写和编辑需要 `posix-rename@openssh.com`；不支持时会明确失败。按版本覆写的复核不是原子 CAS，并发写入可能在复核后发生。SSH 无法查询 PTY 前台进程组或证明远端进程树已终止，前台检查不可用，信号和终止请求在状态不明时会失败；Windows 远端不支持基础模式的命令与 PTY。Code Mode 在桌面 helper 本机的隔离 Goja 进程中运行，只有声明的远端工具 binding，没有 Node fallback 或本机工作区访问。
- **仅 Agent 模式需要目标账号获准 TCP 转发**：SSH 登录成功后，服务器仍可能拒绝 agent 健康检查所需的转发；向导会提示登录已成功但服务器拒绝 TCP 转发（`port-forwarding-denied`）。请管理员核对适用于该账号的 `AllowTcpForwarding`、`DisableForwarding`、`PermitOpen` 和 `Match` 限制，按服务器策略调整并应用配置后重试，或改用获准转发的账号；无需全局放宽转发策略。基础模式不依赖这项授权。
