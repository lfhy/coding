...
describe('BrandMark', () => {
  it('renders the Coding image at a square requested size', () => {
    const { container } = render(<primitives.BrandMark size={34} className="brand" />)
    ...
    expect(image.getAttribute('class')).toBe('brand')
  })
})

describe('BrandWordmark', () => {
  it('renders the Coding name artwork at a stable geometry', () => {
    const view = render(<primitives.BrandWordmark />)
    const svg = view.container.querySelector('svg')!
    expect(svg.getAttribute('width')).toBe('74')
    expect(svg.getAttribute('viewBox')).toBe('0 0 74 24')
