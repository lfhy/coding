...
Remote-SSH 向导必须复用设置面板的容器语言：宽度优先使用 800px 的约束，单一 layer-2 表面、lv3 阴影和 24px 圆角；步骤栏使用透明底色，步骤项与设置导航项同构（图标 + 标题，40px 行高，无副标题），当前步骤使用导航选中填充。字段边框、目录列表边界和主机密钥警告只在表达输入、滚动或风险时保留，不得再套一层装饰性面板。实现位于 [`RemoteSshWizard.module.css`](ui-workspace/src/client/RemoteSshWizard.module.css)。
...
所有文本类 `input`、`textarea` 和 `select` 的键盘焦点必须清晰可见，沿控件圆角呈现，或由圆角父容器以 `focus-within` 呈现；圆角字段内部不得出现方角焦点框，也不得叠加两层焦点边框。复选框、单选框和按钮仍须保留各自的焦点反馈。
...
桌面面板在视口四周保留至少 24px 安全边距；窄屏改为 `calc(100vw - 24px)`，内容列允许滚动而不溢出。Remote-SSH 在 620px 以下把步骤导航改为横向滚动条，表单和操作区保持可触达。
