...
export interface ProcessInspector {
  ...
  /**
   * 判断前台进程组是否在读取终端 shell 的标准输入。
   * @param pgid - 前台进程组标识。
   * @param shellPid - 持久终端 shell 的进程标识。
   * @returns 某个组成员是否被阻塞在 shell 的终端输入上。
   */
...
// `/proc/<pid>/stat` 的 tty_nr 是带符号 32 位设备号，Node 的 st_rdev 则保留同一位模式的非负值。
function linuxDeviceNumber(value: number): number {
  return value >>> 0
}

// `/dev/tty` 的 stat 指向别名而不是选中的 PTY，需以进程 tty_nr 作为可比较的终端身份。
...
// `/proc/<pid>/task/<tid>/syscall` 使用内核 ABI 编号；用户态模拟可能与 process.arch 对应的 ABI 不同。
