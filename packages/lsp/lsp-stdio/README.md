...
- Validates every server-local setting before registration; an invalid mapping or registration conflict rolls back earlier entries, so a failed load leaves no provider routes. Executable lookup waits for the workspace that selects its execution world.
...
- Resolves the server executable, cwd, process, and protocol streams through `ctx.subprocess` for each canonical workspace target; a Remote-SSH marker therefore starts the configured server on the selected remote target, not through the local Node Host. `initialize.processId` is `null` because another machine or PID namespace must not monitor the harness process.
...
|---|---|---|
| `command` | (required) | Executable to spawn — absolute, or resolved on the selected workspace's child PATH on its first query. Launch uses no shell. |
...
`servers` must contain at least one entry, and every id must be non-empty. Timer budgets must be positive integers no greater than Node's `2_147_483_647` ms timer limit. Configuration errors prevent every provider from registering; executable lookup happens after credential scrubbing when the first matching workspace query selects an execution world. Processes launch lazily on that query.
