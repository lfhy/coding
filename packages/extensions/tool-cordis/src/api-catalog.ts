...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Resolve one preset by id. A broken preset resolves — deleting one, reading one, and reporting one all need the row — and the mounting paths refuse it AFTER resolution through resolveMountable.',
      ...
      },
      {
        ...
        description: 'The preset one live agent runs on. Read from the live scope chain rather than from the session, so it answers for an agent whose session has not recorded a preset yet — a child agent whose durable header is being built from its parent\'s composition.',
      ...
      },
      {
        ...
        description: 'One agent\'s instance of a service its preset mounted. A preset publishes services behind `isolate` realms, which are invisible outside the group that declares them — including to the host. This is how a caller holding the agent reads one anyway: a request that is ABOUT a session but arrives from outside it, which is every browser RPC. Read addressing only. A host row that `inject`s a service cannot use this, because injection resolves before any session exists and has no agent to key by; such a service belongs on the host plane instead.',
      ...
      },
      {
        ...
        description: 'The standing scope key of one preset, for a host reader with no agent. A cold transcript read resolves tool presenters against the composition the session recorded, and the standing mount makes that possible without resuming anything: ensuring the mount composes plugins but starts no agent, no session, and no turn.',
        parameters: [{ name: 'id', description: 'the preset id, or `undefined` for {@link defaultId}.' }],
  ...
  },
  {
    ...
    methods: [
      {
        signature: 'abstract resolveExecutable( command: string, env?: Readonly<Record<string, string>>, signal?: AbortSignal, remoteTarget?: RemoteWorkspaceTarget, ): Promise<string>',
        ...
        parameters: [{ name: 'command', description: 'absolute executable path or bare PATH name.' }, { name: 'env', description: 'explicit environment entries used for lookup.' }, { name: 'signal', description: 'aborts remote or local lookup.' }, { name: 'remoteTarget', description: '可选的已验证 Remote-SSH 执行身份。' }],
      ...
      },
      {
        ...
        description: '分配真实控制终端并启动一个由提供方管理的进程会话。这是唯一的非 pipe 进程原语：实现负责终端字节 I/O、前台控制身份、终端特定控制动作及完整 会话树清理。',
        parameters: [{ name: 'spec', description: '完全指定的 argv、cwd、环境、尺寸、宽限期与分配取消。' }],
    ...
    methods: [
      {
        ...
        description: 'Present the calling scope\'s tools in `mode` instead of the deployment default. Nearest scope on the chain wins, so a preset\'s standing declaration covers every agent joined under it. Scoped only, and one declaration per scope: this is how an agent preset composes Code Mode agents beside native ones in the same process, and a process-global override would be the `mode` config field instead.',
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    parameters: [{ name: 'options', description: 'the full request. A LOOP-built request carries the process-local {@link markAgentLoopRequest} identity and arrives deep-frozen (mutation throws): its content is a pure function of the session log (the reconstructability design record), so listeners read it, never rewrite it. Hand-built calls do not carry that marker; their messages already obey the immutable creation contract.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  },
  {
    ...
    declaration: 'export interface RemoteWorkspaceTarget extends RemoteWorkspace {\n    remotePath: string;\n}',
  },
  {
    name: 'ReplayEnvelope',
