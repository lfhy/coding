...
该拆分是一个标准的能力 seam（capability-seams 设计记录）：`dsh-bash-sandbox` 是位于同一 Service Definition 之后的沙箱执行器——Consumer 检测其 `sandboxMode` 能力并添加升权字段，无需导入提供方——容器化或远程执行器也可以同样接入。
...
- **前台超时始终由执行器负责**：seam 上由调用方负责 deadline 的模式已由 工具调用超时策略 设计记录 明确暂缓。
