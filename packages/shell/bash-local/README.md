...
- **Desktop Remote-SSH foreground adapter** — when `run()` receives a cwd beneath a current Remote-SSH marker, it sends the mapped workdir, `bash -c` command, scrubbed environment, timeout, and output budget through the authenticated bridge. It waits for one bounded terminal result and never starts the command locally. `start()` rejects the marker workspace because the gateway does not implement a publishable background-process handle.
...
- **Remote execution is foreground-only** — the bounded bridge has no streamed output, spill file, background handle, persistent shell, PTY, or generic subprocess lifecycle. The sandboxed desktop composition additionally requires `danger-full-access` for every remote Bash call.
