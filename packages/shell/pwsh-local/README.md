...
- **Remote-SSH target selection** — a current marker workdir selects the target Go agent for foreground and background processes. A configured `pwshPath` remains explicit; otherwise the agent resolves bare `pwsh` on the target instead of reusing the Host's resolved absolute path. The Host retains timeout/cancel classification, job ownership, and model-visible rendering.
...
- **Remote PowerShell depends on the target** — the target must provide the selected explicit executable or a `pwsh` available on its PATH; a broken marker or bridge fails without a local fallback.
