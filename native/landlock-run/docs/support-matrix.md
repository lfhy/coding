...
| Platform package | Native build host | Notes |
|---|---|---|
| `@deepseek-ai/node-addon-landlock-run-linux-x64` | Linux x64 with `musl-gcc` | static musl — glibc and musl distros alike |
| `@deepseek-ai/node-addon-landlock-run-linux-arm64` | Linux arm64 with `musl-gcc` | static musl — glibc and musl distros alike |
...
- **Other Linux architectures** (riscv64, s390x, …): no maintained native build target yet. The no-cross-toolchain rule means a platform package is added only together with a native host that builds and proves it.

A consumer on an unsupported platform resolves a nonexistent launcher path, probes `unusable`, and falls closed.
