...
本项目是基于 Cordis 的个人 AI 客户端，运行时能力以插件组成。修改 `packages/` 前先阅读 [架构说明](docs/architecture.md)；文档层级、双语配对和预算遵循 [docs/AGENTS.md](docs/AGENTS.md)。项目当前同时建设 CLI 和桌面端，兼容性优先服务本项目的实际使用，不为上游发布流程保留约束。
...
每个包的公开配置、限制、扩展点和用户可见行为写在所属 README；类型、事件和生成目录写在对应文档层级。双语文档必须同时更新并重新记录 `.i18n.yaml`；生成文件修改生成器或源文档后再运行生成命令。非机械的行为、架构、工具链或测试策略变更在同一变更中添加或更新 Agent Note，先检查是否有已实现记录需要归档或交叉引用。
