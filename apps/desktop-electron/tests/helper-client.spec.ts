...
describe('Electron Go helper stdio', () => {
  ...
  it('accepts the first ready frame and correlates responses and progress', async () => {
    ...
    const request = helper.request('RemoteSSHConnect', { attemptId: 'rattempt' })
    const sent = child.sent()
    expect(sent).toMatchObject({ type: 'request', protocol: 1, method: 'RemoteSSHConnect', payload: { attemptId: 'rattempt' } })
    ...
    child.send({ type: 'response', protocol: 1, id: sent.id, ok: true, value: { kind: 'ready' } })
    expect(await request).toEqual({ kind: 'ready' })
