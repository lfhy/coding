...
| 子目录 `AGENTS.md`（`packages/`、`examples/`、`docs/`） | 仅适用于该子树的指令 | 根文件已承载的仓库级规则 |
...
放置规则：缺陷事件写复盘；「在哪改、还要一起改什么」写代码地图；步骤写 cookbook；类型定义写 subsystem；包契约写 README；常驻命令写根 `AGENTS.md` 并链接理由。
...
- 变更历史或 war story：「以前」「现在」「不再」「曾经」「改名」「已移动」、PR 或 commit。改写为当前事实，必要时链接事故复盘。
...
仓库内引用使用相对 Markdown 链接，不写裸文件名。改路径或删文档时自己 grep 入站引用：链接目标、`#fragment` 锚点和代码注释里的 `docs/*.md` 路径都没有门禁兜底。
