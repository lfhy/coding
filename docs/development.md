...
- Node.js supports `^22.19.0 || >=24.0.0`; the personal-client CI uses Node 24.
...
`pnpm run build` embeds the caller's exact `DSH_CLIENT_*` environment and uses no public client values when none are set. `pnpm run build:official` remains available for consumers that require the fixed upstream-compatible profile; the personal-client CI uses the ordinary `pnpm run build` command. Each successful complete build writes a gitignored record that binds those values to the Vite output and dynamic client bundles; built Web tests reject a missing record or artifacts changed by a later partial build.
