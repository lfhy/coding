...
`signal` 是就绪前后唯一的取消通道。subagent 组合控制 设计记录规定 persona、live 全局工具过滤、绝对深度以及「可见性而非权限」的设计理由。
