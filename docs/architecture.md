...
seam 正是替换一个提供方就能改变整个产品的原因。文件系统与进程提供方共享同一个执行世界；桌面端的远程工作区 marker 将请求路由到同一 SSH 连接，并由连接模式限定可用能力：基础模式由 SFTP、SSH 和本机隔离的 Code Mode 承接远端操作，LSP 不可用；Agent 模式通过 Go agent 提供完整远端能力。本地 Host 保留控制平面；marker 不是沙箱，沙箱策略仍由独立提供方负责。具体能力限制见[远程连接指南](user/guide/index.md#选择开始方式)，类型与执行身份见[子进程](subsystems/subprocess.md#可执行文件查找)。[subagent 提供方](subsystems/subagent.md)在同一个接口之后同样千差万别，从新建一个子 agent，到把一个轮次委派给另一个产品。
