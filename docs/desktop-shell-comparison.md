...
本文是 Coding 个人客户端的桌面壳对比与迁移验收参考，不是切换默认入口的承诺。比较范围是窗口、Host 连接、原生能力和本机打包；会话、设置、凭据、LLM 与 UI 业务仍由 [Client 和 Host 插件](architecture.md)拥有。并存原型的运行方式与限制由 [Electron 应用 README](../apps/desktop-electron/README.md)记录。

## 归属与连接

| 事项 | 默认 Wails v2 | Electron 并存原型及后续归属 |
| --- | --- | --- |
| 窗口与启动 | [Go 壳](../apps/desktop/main.go)持有窗口、菜单、托盘、单实例锁和静态启动页；[hostlaunch](../apps/internal/hostlaunch/launcher.go)在启动锁内发现或启动独立 Node Host，校验就绪记录、PID 和 `host.describe`，再把整窗导航到随机 `127.0.0.1` 端口。macOS 关闭窗口只隐藏到托盘，应用退出后共享 Host 仍按自身空闲策略运行。 | 私有 Electron 原型只面向 macOS 开发，使用独立的 home、userData 和工作目录，验证 `host.json` 与 `host.describe` 后加载 Host HTTP origin。单实例、菜单、托盘、共享 Host 退出行为和失败恢复仍须分别验收，不能由窗口启动成功推定。 |
| UI、HTTP 与 WS | [Wails asset server](../apps/desktop/main.go)只提供启动页；导航后，[Web/Host](../packages/client/connection/src/index.ts)直接拥有页面、`/api` HTTP 和 WebSocket，壳不转发 `/api`。Host/Origin/Fetch-Metadata 防护限制跨站和 DNS rebinding，但[不是身份认证](../packages/client/connection/src/api-request-trust.ts)。 | 原型直连经核验的回环 Host origin，不在 main 中转发 `/api` 或 WebSocket；窗口导航、弹窗与权限请求须受限制。直连不能将回环地址当身份认证。HTTP、WS、流式输出、取消、下载和重连仍需实机验证；自定义 scheme 与转发不是本阶段能力。 |
| 原生 IPC | [Wails binding](../apps/desktop/desktop_bindings.go)允许回环 origin 调用，但敏感方法逐次验证窗口随机 token；只有精确匹配就绪 Host origin 才[向 WebView 注入](../apps/desktop/main.go) token 和最小调用面。 | 原型不提供 preload、原生 IPC 或 Browser guest，网页不能借壳调用原生能力。以后引入 IPC 时，main 须逐次核验主窗口、发送 frame、origin 和输入；`contextIsolation`、sandbox 与禁用 renderer Node 不能替代授权。 |
| Remote-SSH | [Go manager](../apps/desktop/internal/remoteagent/manager.go)持有 SSH 连接、host-key 确认和 agent 部署；[Go 回环 bridge](../apps/desktop/remote_bridge.go)只接受 Host 携带独立 bearer token、连接与 marker identity 的请求，Node 不持有 SSH 凭据。退出时先收敛连接再关闭 bridge。 | 原型不接入 Go Remote-SSH manager 或 bridge。后续须定义 Electron main 与 Go helper 的受限进程协议及生命周期；不能用 renderer IPC 或 Host 的 HTTP/WS 通道绕开现有 SSH、marker 和关闭顺序。是否最终改写 Go 能力是另一个决策。 |
...
[Wails 开发入口](../Makefile)使用独立单实例 ID 与 `~/.dsh-dev`，强制从仓库源码启动 Host；安装版使用 `~/.dsh`。[hostlaunch](../apps/internal/hostlaunch/launcher.go)独占写入 `DSH_HOME`、`DSH_CWD`、`DSH_APP_VERSION`，且 Wails 壳为私有 bridge token 更换兼容的旧 Host。Electron 原型的 home、userData 和工作目录与上述实例隔离；它不能复用安装版 Host 或把 API key、SSH 密码、binding/bridge token 放进 URL、renderer 可读持久存储、日志或打包资源。独立开发 home 不自动读取安装版设置和凭据；Electron Host 在隔离工作目录读取 `.env`，不会读取仓库根目录的 `.env`。
...
1. 并存的最小 Electron 原型与独立开发 home：窗口连接经 `host.json`/`host.describe` 验证的 Host，严格限制导航、弹窗与权限；单实例、启动失败展示和 Host 生命周期另行实机验收，开发实例不得触碰安装版数据或凭据。菜单和托盘只在迁移验收时补齐。
